...
class Game {
	...
	preGame() {
		...
		//p1+p2 containers: added color animation
		...
		//p1+p2 inputs: added button click animation
		...
		//startGame-button: added button click animation
	...
	startGame(e) {
		...
		if (this.height < 6 || this.width < 7) {
			msg.innerHTML = 'Please create a bigger board';
			msgContainer.style.opacity = '0.95';
			return;
		} else if (this.p1.color === this.p2.color) {
			msg.innerHTML = 'Please select different colors from your opponent';
			msgContainer.style.opacity = '0.95';
			return;
		} else if (this.p1.color === '#ffffff' || this.p2.color === '#ffffff') {
			msg.innerHTML = 'P1 or P2: Please select a color that is NOT white';
			msgContainer.style.opacity = '0.95';
			return;
		} else {
			msg.innerHTML = '';
	...
	handleClick(e) {
		//handleClick will turn off each column, once all empty spots are filled in that column
